...
# ── Baca konfigurasi DeepSeek dari .env ─────────────────────────────────────
BASE_URL = os.getenv("TEST_BASE_URL", "https://api.deepseek.com/v1")
API_KEY = os.getenv("TEST_API_KEY")
MODEL = os.getenv("TEST_MODEL", "deepseek-chat")

if not API_KEY:
    raise EnvironmentError(
        "❌ TEST_API_KEY tidak ditemukan di .env! "
        "Pastikan sudah mengisi TEST_API_KEY=sk-deepseek-xxx"
    )


def main():
    input_file = "scenario1_data.json"
    output_file = "answersDeep.json"
    ...
    # 3. Inisialisasi client DeepSeek (kompatibel OpenAI SDK)
    client = OpenAI(
        base_url=BASE_URL,
        api_key=API_KEY,
    )
    ...
    for item in source_data:
        ...
        try:
            ...
            print("🛑 Memberhentikan eksekusi sementara. Silakan coba lagi nanti.")
            break
        ...
        # Jeda 1 detik (DeepSeek biasanya lebih longgar dari OpenRouter)
        time.sleep(1)
    ...
    if len(processed_ids) < len(source_data):
        print(
            "💡 Ketik ulang `python task2_run_deepseek.py` untuk melanjutkan sisa soal!"
        )
