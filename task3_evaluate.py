...
class OpenRouterJudge(DeepEvalBaseLLM):
    def __init__(self):
        self.model = ChatOpenAI(
            model=os.getenv("JUDGE_MODEL", "openai/gpt-4"),
...
def main():
    ...
    if os.path.exists(output_file):
        with open(output_file, "r", encoding="utf-8") as f:
            try:
                ...
                print("⚠️ File evaluation_results.json korup/kosong. Membuat baru...")
                evaluated_results = []
    ...
    print(f"\n⏳ Memulai evaluasi untuk {len(batch_items)} soal pada sesi ini...")
    ...
    # ---------------------------------------------------------
    # 3. EVALUASI RAGAS (Jalankan sekaligus untuk 1 batch)
    # ---------------------------------------------------------
    print("\n--- 📐 Memulai Evaluasi RAGAS (Batch) ---")
    ragas_dataset_dict = {
        "question": [item["query"] for item in batch_items],
        "answer": [item.get("actual_output", "") for item in batch_items],
        "ground_truth": [item.get("expected_output", "") for item in batch_items],
        "contexts": [[item.get("source", "(tidak ada)")] for item in batch_items],
    }
    ...
    try:
        ...
        print("✅ RAGAS selesai untuk batch ini!")
    except Exception as e:
        print(f"❌ Error API RAGAS: {e}")
        print(
            "🛑 Eksekusi diberhentikan. Pastikan limit API Anda belum habis, lalu coba lagi."
        )
        return

    # ---------------------------------------------------------
    # 4. EVALUASI DEEPEVAL & AUTO-SAVE
    # ---------------------------------------------------------
    print("\n--- 🔬 Memulai Evaluasi DeepEval & Auto-Save ---")

    for i, item in enumerate(batch_items):
        print(f"   Mengevaluasi DeepEval untuk ID {item['query_id']} ...")
        r_scores = ragas_df.iloc[i]
        ...
        # Gabungkan RAGAS & DeepEval
        item["evaluations"] = {
            "ragas_answer_relevancy": float(r_scores.get("answer_relevancy", 0) or 0),
            "ragas_faithfulness": float(r_scores.get("faithfulness", 0) or 0),
            "ragas_answer_correctness": float(
                r_scores.get("answer_correctness", 0) or 0
            ),
    ...
    if len(evaluated_results) < len(source_data):
        print(
            "💡 Ketik ulang `python task3_evaluate.py` untuk melanjutkan evaluasi sisa soal!"
        )
